...
class Trainer:
    ...
    def fit(self, train_dataloader, val_dataloader):
        ...
        """
        if hasattr(self.model, "fit_drift_buckets_from_dataset"):
            drift_bucket_stats = self.model.fit_drift_buckets_from_dataset(
                getattr(train_dataloader, "dataset", None)
            )
            if drift_bucket_stats is not None and self.accelerator.is_main_process:
                self.log(f'[Drift MoE] Fitted train drift buckets: {drift_bucket_stats}')
