...
class DADRecTrainer:
    ...
    def __init__(self, config: dict, model: AbstractModel, tokenizer: AbstractTokenizer):
        ...
        # 设置保存路径
        ckpt_name = get_file_name(config, suffix='')
        self.saved_model_ckpt = os.path.join(
            self.config.get('ckpt_dir', 'ckpt/'),
            f'{ckpt_name}_pid{os.getpid()}.pth'
        )
    ...
    def evaluate(self, dataloader, split='test'):
        ...
        evaluator = DADRecEvaluator(self.config, self.tokenizer)
