...
def collate_fn_train(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    if all('target_sem_emb' in b for b in batch):
        out['target_sem_emb'] = stack_to_tensor(
            [b['target_sem_emb'] for b in batch],
            dtype=torch.float32
        )
    if all('target_opq_subvectors' in b for b in batch):
        out['target_opq_subvectors'] = stack_to_tensor(
            [b['target_opq_subvectors'] for b in batch],
            dtype=torch.float32
        )
    return out
